...
<head>
    ...
    <title>Generate Selection</title>
    <style>
        table {
...
<body>
    ...
    while ($i < ($pref * $subcnt)) {
        if ($taid[$i] != -1) {
            $getrowsql = "SELECT name FROM ta WHERE sno=" . $taid[$i];
    ...
    for ($i = 1; $i <= $tacnt; $i += 1) {
        if (in_array($i, $taid) == false) {
            $getrowsql = "SELECT name FROM ta WHERE sno=" . $i;
            $newresult = mysqli_query($conn, $getrowsql);
            if ($newresult) {
                $rowdata = mysqli_fetch_row($newresult);
            ...
            $sql = "SELECT * FROM ta where sno=" . $i;
            $result = mysqli_query($conn, $sql);
